...
public class CommonHelper {
	...
	/**
	 * Returns a byte array of the supplied inputStream that can be saved to a
	 * blob database table field. This method will return a null if the
	 * parameter is null
	 * 
	 * @param inputStream
	...
	public static boolean hasValidValue(final Object obj) {
		...
		if (obj != null) {
			if (obj.getClass() == String.class) {
				...
				if (strValue.trim().length() != 0 && strValue.equalsIgnoreCase(Constants.EMPTY) == false) {
					return true;
				}
	...
	/**
	 * Extract a valid value from a String, if the object is null it will return
	 ...
	 * 
	 * @param speed
	 *            - Speed, obj will be converted to string, '1G', '200M',
	 *            '512kb'
	...
	/**
	 * Adds n number of days to a certain timestamp, regardless of whether
	 * weekday or weekend
	 * 
	 * @param timestamp
	 *            - days to add the start date
	...
	public static Timestamp minusDayss(Timestamp baseDate, int days, boolean inWorkingDays) {
		if (baseDate != null && days > 0) {
			...
			while (index <= days) {
				dateCal.add(Calendar.DAY_OF_MONTH, -1);
				if (inWorkingDays || (dateCal.get(Calendar.DAY_OF_WEEK) != Calendar.SATURDAY && dateCal.get(Calendar.DAY_OF_WEEK) != Calendar.SUNDAY)) {
					index += 1;
				}
			}
			return new Timestamp(dateCal.getTimeInMillis());
	 ...
	 *            - max value
	 * 
	 *            if Current value is greater than the max value it will be
	 *            switched
	 * 
	 * @return
	 */
	...
	/**
	 * Specify the range by integer range. Method will return the values from
	 * the range give.
	 * 
	 * @param longList
	 * @param from
	...
	/**
	 * Return a boolean string flag that is used to determine if flag Active.
	 * Case insensitive Matching
	 * 
	 * @param booleanStr
	...
	/**
	 * Removes spaces, alphabets or special characters using regex so that only
	 * numbers will remain
	 * 
	 * @param numberStr
	...
	private static synchronized Properties getConfigPropInstance() {
		if (configProp == null) {
			try {
				...
				final FileInputStream configFile = new FileInputStream(Constants.CONFIG_PATH);
	...
	/**
	 *  Reads from the configuration file of the application and returns the value as a String.
	 *  Path to the Config file is defined on net.virtela.utils.Constants field CONFIG_PATH
	 * 
	 * @param configKey
	 * @return
	 */
